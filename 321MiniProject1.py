...
import math  # "math" library imported in order to use the natural log(ln) function.
import sys  # "sys" imported since the program exits if there is any unrealistic value between the inputs.
...
#Here, we import the excel file that we prepared for some constant values to the source code.
import pandas as pd  
data=pd.read_excel('pythontable.xlsx',header=None,index_col=0,skiprows=1)
properties=data.values

T_C = 25  # This is the temperature which the reactions are occured at 25°C.
T = T_C + 273.15  # This is the temperature which the reactions are occured at 298.15K.

def MCPH(A, B, C, D, T, T0):
    
    # MCPH function calculates the mean heat capacity value specific to enthalpy calculations.
    # A, B, C, D are the heat capacity constants.
    # T is the final, T0 is the inital temperature. 
...
def mean_heat_capacity_S(A, B, C, D, T, T0):
    ...
    # This function calculates the mean heat capacity value specific to entropy calculations.
    # A, B, C, D are the heat capacity constants.
    # T is the final, T0 is the inital temperature.
...
def standard_entropy(n, CO2, H2O, a, O2):
    ...
    # The 'a' parameter is reffering the alkane in this chemical equation.
    # n is the mole number of the alkane burned.
    # All CO2, H2O, a, and O2 parameters mean its stoichiometric coefficient times the standard entropy of that substance in the gas form.
    # we read the standard entropy values from Table 2-221 in Perry's Chemical Engineering Handbook.
...
# Firstly, inputs about calculation type and substance type are requested from user

"""
The first four if loops check whether user inputs are in correct form or not.
        ...
        if mole_of_alkane > 0:
            ...
            if 100 >= excess_air >= 0:
                ...
                if calculation_type == "T":
                    ...
                    T0_a_K = T0_a_C + 273.15 # Conversion from °C to K.
                    
                    # These two if statements below check whether user inputs are realistic or not.
                    
                    if T0_a_K >= 0:
                        
                        T0_air_C = float(input("\nPlease enter the initial temperature of the air(°C): "))
                        T0_air_K = T0_air_C + 273.15 # Conversion from °C to K.
                        ...
                        if T0_air_K >= 0:
                            ...
                            if burned_alkane == "E":
                                ...
                                # These are the stoichiometric coefficients of the ethane combustion reaction.                              
                                coef_ethane = 1
                                coef_O2 = 7/2
                                coef_CO2 = 2
                                coef_H2O = 3                                
                                
                                #These are the molar quantities of the feed.
                                ...
                                #These are the molar quantities of the products.
                                ...
                                # These are the heat capacity constants for air.
                                ...
                                # These are the heat capacity constants for ethane.                              
                                ...
                                # These are the heat capacity constants for products.                                
                                ...
                                tolerance = 10**-4  # The convergence tolerance of the result gotten from the iteration method.
                                T_guess = 2*T  # It is initial guess for the flame temperature.
                                error = 100  # For the initial computation in the while loop, 100 is merely a random value that satisfies error>tol.
                                
                                #  This while loop contains the iteration and calculates the adiabatic flame temperature.
